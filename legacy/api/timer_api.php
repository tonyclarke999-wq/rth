...
# Timer API
#
# $RCSfile: timer_api.php,v $ $Revision: 1.2 $
# ------------------------------------
		...
		function print_times() {
			...
			if ( ON == $g_debug_timer ) {
				...
				for ($i=0;$i+1<count($this->atime);$i++) {
					...
					print"<tr>". NEWLINE;
					print"	<td class=footer-l>Time: ".number_format( $time, 6 )." seconds ( ".number_format( $time_precent, 2 )."% ) for ".$this->atime[$i][0]." -to- ".$this->atime[$i+1][0]."</td>". NEWLINE;
					print"</tr>". NEWLINE;
				}
			}
			...
			# display total time
			print"<tr>". NEWLINE;
			print"	<td class=footer-l>Time: ".number_format( $total_time, 6 )." seconds.</td>". NEWLINE;
			print"</tr>". NEWLINE;
